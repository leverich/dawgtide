...
for i in `python generate_weeks.py`; do
    if [ ! -f "SFB1301_12_$i.csv" ]; then
        curl "http://tidesandcurrents.noaa.gov/noaacurrents/DownloadPredictions?fmt=csv&id=SFB1301_12&d=$i&r=2&t=24hr&u=1&tz=GMT&i=6min" -o SFB1301_12_$i.csv
    fi
done
